...
namespace RS.Utils
{
    public class SplineSampler : RsSampler
    {
        ...
        private float SampleSingle(Vector3 pos, float x)
        {
            var pointCount = m_values.Length;
        ...
        public override NativeArray<float> SampleBatch(Vector3[] posList)
        {
            ...
            var sampleResult = new List<NativeArray<float>>();
            for (var i = 0; i < pointCount; i++)
            {
                if (indexToSample[i].Count == 0)
                {
                    sampleResult.Add(new NativeArray<float>(0, Allocator.TempJob));
                    continue;
                }
                sampleResult.Add(m_values[i].SampleBatch(indexToSample[i].ToArray()));
            }
